...
func main() {
	fmt.Println("Resultado de correr vectors()")
	//vectors()
	fmt.Println()
	fmt.Println()
	fmt.Println("Resultado de correr matrix()")
	matrix()
	fmt.Println()
	fmt.Println()
}
...
func vectors() {
	...
	vectorB := mat.NewVecDense(3, []float64{4, 5, 6}) // if dimension and array values don't match gives error
...
func matrix() {
	...
	formatterB := mat.Formatted(matBB, mat.Prefix(""))
	fmt.Printf("%v\n", formatterB)
